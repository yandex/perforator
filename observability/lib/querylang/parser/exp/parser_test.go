...
func TestParseExpression(t *testing.T) {
	for _, tc := range []testCase{
		...
		{
			Query:        `{project="logging"} | filter(x->eq(get(x, 'a'), 'b'))`,
			ExpectedStr:  `filter({"project" = "logging"}, x -> eq(get(x, "a"), "b"))`,
			ExpectedRepr: `filter({"project" = "logging"}, (x) -> eq(get(x, "a"), "b"))`,
		},
		{
		...
		{
			Query:         `filter(, x->eq(get(x, 'a'), 'b'))`,
			ExpectedError: `unexpected token '(' (at line 1, column 6)`,
		},
		{
			Query:         `{a = b} | filter(x->eq(get(x, 'm'), sent metrics))`,
			ExpectedError: `unexpected token '|' (at line 1, column 8)`,
		},
	} {
		t.Run(tc.Query, func(t *testing.T) {
			p := parserexp.NewParser()
