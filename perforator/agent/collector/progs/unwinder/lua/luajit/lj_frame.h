...
#define frame_ftsz(f)		((ptrdiff_t)BPF_PROBE_READ_USER(f, ftsz))
...
#define frame_contv(f)		(BPF_PROBE_READ_USER((f)-1, u32).lo)
#endif
#if LJ_FR2
...
#define frame_prevl(f)		((f) - (1+LJ_FR2+bc_a(BPF_PROBE_READ_USER_POINTER(&frame_pc(f)[-1]))))
...
#define cframe_L(cf) \
  (&gcref(BPF_PROBE_READ_USER_POINTER((GCRef *)(((char *)(cf))+CFRAME_OFS_L)))->th)
#define cframe_pc(cf) \
  (mref(BPF_PROBE_READ_USER_POINTER((MRef *)(((char *)(cf))+CFRAME_OFS_PC)), const BCIns))
