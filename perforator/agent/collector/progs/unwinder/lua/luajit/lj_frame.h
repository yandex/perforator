...
#define frame_prevl(f)		((f) - (1+LJ_FR2+bc_a(BPF_PROBE_READ_USER_FROM(&frame_pc(f)[-1]))))
...
#define cframe_L(cf) \
  (&gcref(BPF_PROBE_READ_USER_FROM((GCRef *)(((char *)(cf))+CFRAME_OFS_L)))->th)
#define cframe_pc(cf) \
  (mref(BPF_PROBE_READ_USER_FROM((MRef *)(((char *)(cf))+CFRAME_OFS_PC)), const BCIns))
