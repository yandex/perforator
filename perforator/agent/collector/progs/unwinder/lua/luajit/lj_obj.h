...
#define proto_kgc(pt, idx) \
  check_exp((uintptr_t)(intptr_t)(idx) >= ~(uintptr_t)BPF_PROBE_READ_USER(pt, sizekgc)+1u, \
	    gcref(BPF_PROBE_READ_USER_FROM(&mref(BPF_PROBE_READ_USER(pt, k), GCRef)[(idx)])))
...
#define mmname_str(g, mm)	(strref(BPF_PROBE_READ_USER_FROM(&(g)->gcroot[GCROOT_MMNAME+(mm)])))
