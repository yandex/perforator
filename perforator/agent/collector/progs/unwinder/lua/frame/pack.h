 ...
 *
 * `struct symbol_key` has limited space and we need to fit Lua, C, FF and
 * invalid frames into it!
 *
 *  - Lua frame
 *
...
enum {
    // In canonical form in most architectures userspace addresses are 48 bits
    // long.
    ...
    LUA_OBJECT_ADDRESS_STACK_WALK_ERROR_BITS = 2,

    // Offset for gct in invalid frame.
    LUA_OBJECT_ADDRESS_GCT_OFFSET =
        LUA_OBJECT_ADDRESS_PTR_BITS + LUA_OBJECT_ADDRESS_STACK_WALK_ERROR_BITS,
...
lua_frame_pack_invalid_object_address(enum lua_stack_walk_error error, u8 gct) {
    ...
    object_address |=
        ((u64)error << LUA_OBJECT_ADDRESS_STACK_WALK_ERROR_OFFSET);
