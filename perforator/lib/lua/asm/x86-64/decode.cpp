...
TMaybe<ThreadImageOffsetType> DecodeLuaClose(
    ...
    NPerforator::NAsm::DecodeInstructions(TLoggerOperator<TGlobalLog>::Log(), triple, bytecode, [&](const llvm::MCInst& inst, ui64 size) {
        ...
        case llvm::X86::MOV8mi: {
            ...
            if (base.isReg() && base.getReg() == Greg && disp.isImm() && imm.isImm() && imm.getImm() == 0) {
                // stop the search
                result = disp.getImm();
                return false;
            }
...
TMaybe<ThreadImageOffsetType> DecodeLjDispatchUpdate(
    ...
    auto mri = std::unique_ptr<llvm::MCRegisterInfo>(target->createMCRegInfo(triple.getTriple()));
