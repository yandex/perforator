...
enum class ELuaVersionSource {
    ...
    // expose `luaJIT_version_*` symbol. Custom builds might remove this symbol
    // as well. `lua_version` symbol is useless, as it always returns constant
    // value `LUA_VERSION_NUM` (501). For now just hard-code 2.1.0 version if
    // the binary happen to expose `lua_close` and `luaopen_jit` or
    // `luaopen_bit`, should be enough for current purposes
    LuaJitDeduced,
};
...
class TLuaAnalyzer {
  public:
    struct TLuaSymbols {
        ...
        TMaybe<NPerforator::NELF::TLocation> LuaOpenJit; // `luaopen_jit` TODO: find offsets without this symbol?
